    ...
    public:
        ...
        Intent(nlohmann::json j, nlohmann::json em){
            ...
            for (auto& elem: j["entities"]){
                ...
                Entity *e = new Entity(elem, em[en]);
                listEntity.push_back(e);
            }
        }
...
int main(int argc, char** argv){
    ...
    options.add_options()
    ("i,ip", "Server IP Address", cxxopts::value<std::string>()->default_value("127.0.0.1"))
    ("p,port", "Service Port, Default value is 3000", cxxopts::value<std::string>()->default_value("3000"))
    ("w,worker", "Number of max worker for parallel requesting", cxxopts::value<int>()->default_value("10"))
    ("d,data", "Path to data source", cxxopts::value<std::string>()->default_value("data"))
    ("n,conversation_number", "number of conversation", cxxopts::value<int>()->default_value("50"))
    ;
